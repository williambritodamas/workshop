...
interface QuizMixerProps {
  questions: QuizQuestion[];
}

export const QuizMixer: React.FC<QuizMixerProps> = ({ questions }) => {
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2 text-blue-400">
            ...
            <h3 className="text-lg font-bold text-white">Que controle é este?</h3>
          </div>
          <span className="text-xs text-slate-500 font-mono">{current + 1}/{questions.length}</span>
        </div>

        <div className="relative h-48 md:h-56 rounded-2xl overflow-hidden mb-4 border border-slate-800 bg-slate-950 flex items-center justify-center">
          <div className="text-center p-6">
            <div className="w-full h-full flex items-center justify-center">
              <div className={`p-8 rounded-2xl bg-slate-900 border-2 ${revealed && isCorrect ? 'border-emerald-500' : 'border-blue-500/50'}`}>
                <span className="text-2xl md:text-3xl font-black text-white">{q.label}</span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          {q.options.map((opt, i) => {
            ...
            if (revealed && i === q.correct) btnClass = 'border-emerald-500 bg-emerald-500/20 text-emerald-300';
            else if (selected === i && !revealed) btnClass = 'border-blue-500 bg-blue-500/20 text-blue-300';
            return (
              <button
                key={i}
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleReveal}
            className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm transition-all shadow-lg cursor-pointer"
          >
