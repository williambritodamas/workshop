...
export const Slide02_Question: React.FC = () => (
  <div className="relative w-full flex-1 flex flex-col justify-center items-center text-center p-8 md:p-16 overflow-y-auto">
    ...
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0">
      <div className="w-[400px] h-[400px] rounded-full border border-amber-500/20 animate-wave" />
      <div className="w-[600px] h-[600px] rounded-full border border-amber-500/10 animate-wave delay-500" />
    </div>
    <div className="relative z-10 max-w-4xl flex flex-col items-center">
      <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, type: 'spring' }}
        className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-400 mb-6"
      >
        <HelpCircle className="w-12 h-12 md:w-16 md:h-16" />
      </motion.div>
      <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }}
        className="text-5xl md:text-7xl lg:text-8xl font-black text-white tracking-tight drop-shadow-lg"
      >
        Gain e Volume<br/>são a <span className="text-amber-500">mesma coisa?</span>
      </motion.h1>
      ...
      >
        <p className="text-slate-400">Essa é a dúvida #1 de quem está começando.</p>
        <p className="mt-2 text-white font-medium">A resposta pode salvar seu som (e seus alto-falantes).</p>
      </motion.div>
    </div>
  </div>
);
